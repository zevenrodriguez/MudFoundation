...
//
//   // In update loop, once per active pointer (mouse, or each XR hand):
//   btn.update(pointerId, raycast, isPressed, isReleased);
// ─────────────────────────────────────────────────────────────────────────────
function MudButton(options) {
    ...
    // RayInteractor is multi-pointer aware (see RaycastInteractorObject.js) —
    // guard onExit with isHovered() so a button being touched by two pointers
    // at once (e.g. both XR controllers) doesn't flicker back to "normal"
    // just because one of them left.
    interactor.onEnter = function() { base._draw(hoverBg, hoverBorder); };
    interactor.onExit  = function() {
        if (interactor.isHovered()) return;
    ...
    const self = {
        ...
        update: function(pointerId, raycast, isPressed, isReleased) {
            interactor.update(pointerId, raycast, isPressed, isReleased);
        },
        reset: function(pointerId) {
    ...
    interactor.onClick = function(hit, pointerId) { self.onClick(hit, pointerId); };
