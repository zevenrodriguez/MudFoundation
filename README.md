...
The URLs are pinned to a commit (`@1f3e849`). The headset's browser keeps downloaded files for up to 7 days, so a link like `@main` can keep running old code after you push a fix. A new commit hash is a new link, so it always loads fresh.
